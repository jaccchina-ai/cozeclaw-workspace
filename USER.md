# USER.md - About Your Human

*Learn about the person you're helping. Update this as you go.*

---

## 1. Identity

| Attribute | Value |
|-----------|-------|
| **Name** | 老板 |
| **What to call them** | 老板 |
| **Pronouns** | 您 |
| **Timezone** | Asia/Shanghai (中国标准时间) |
| **Organization** | JACC Office Machine Co., Ltd. (abbreviated as "JACC")|
| **Industry** | Manufacturing, AI Automation, Investment Research |

---

## 2. Role & Mission

### Role
Business Executive / AI System Architect

### Primary Mission
Build and operate an AI-driven business system that automates:
- Sales and customer follow-ups
- Market research
- Investment strategy analysis
- Business process automation

### Secondary Mission
Act as a strategic advisor that proposes optimizations, tools, and workflows.

### Primary Operating Mode
Strategic AI Co-Pilot for business, trading, and automation systems.

---

## 3. Current Focus Areas

| Project | Status | Priority |
|---------|--------|----------|
| OpenClaw AGI architecture | 🔄 Active | High |
| A-share stock selection AI system (T01) | 🔄 Active | High |
| B2B customer follow-up automation | 📋 Planned | Medium |
| AI-driven marketing and research automation | 📋 Planned | Medium |

---

## 4. Decision Authority

| Action Type | Permission |
|-------------|------------|
| Suggest ideas | ✅ Allowed |
| Analyze strategies | ✅ Allowed |
| Generate plans | ✅ Allowed |
| Execute high-impact changes | ⚠️ Require approval |
| Modify system architecture | ⚠️ Require approval |
| Financial/trading decisions | ⚠️ Require approval |

---

## 5. Preferences

### Communication Style
| Preference | Setting |
|------------|---------|
| **Response Length** | Detailed, but "conclusion first, then reasoning" |
| **Code Style** | Python-first, clear structure, well-commented |
| **Information Density** | Concise summaries first, detailed analysis when needed |

### Work Habits
| Habit | Setting |
|-------|---------|
| **High-efficiency period** | Night time |
| **Communication preference** | Async-first, real-time for urgent matters |

### Expected Agent Personality
- **Professional**: Maintain professional standards, high-quality output
- **Rigorous**: All information must be accurate, verify data sources
- **No fabrication**: Mark uncertain content as "pending confirmation", never fabricate

---

## 6. Preferred Tools

| Category | Tools |
|----------|-------|
| **Programming** | Python (priority), Node.js |
| **Automation** | Playwright, OpenClaw |
| **AI/LLM** | OpenRouter, Together AI, DeepSeek, GLM, Kimi, Gemini, ChatGPT |
| **Version Control** | GitHub |
| **Communication** | Feishu, DingTalk, Email |

---

## 7. Risk Profile

| Parameter | Setting |
|-----------|---------|
| **Risk Tolerance** | Balanced |

### Guidelines:
- ✅ Experimentation allowed in sandbox
- ⚠️ Production systems require verification
- 🚨 Critical financial decisions require human confirmation

---

## 8. Proactive Monitoring Interests

| Category | Items |
|----------|-------|
| **AI/Technology** | New AI models, LLM releases, AI tools |
| **Development** | GitHub projects related to AI agents |
| **Finance** | Financial market signals, market changes |
| **Productivity** | Automation tools, productivity improvements |

---

## 9. Check-in Frequency

**Daily summary preferred.**

Provide:
- 📊 Important updates
- ⚠️ Potential risks
- ✨ New opportunities

---

## 10. Strategic Vision

**Long-term objective (6–12 months):**

Build a fully integrated AI operating system capable of supporting:
- 📈 Automated trading research
- 👥 Autonomous customer development
- 📊 Automated business intelligence
- 🧠 AI-assisted decision making

---

## 11. Goals

### Short-term (30 Days)
- [x] Complete OpenClaw base configuration
- [x] Configure Feishu, DingTalk channels
- [x] Install proactive-agent and self-improvement skills
- [x] Configure vector search (OpenRouter embeddings)
- [x] Complete ONBOARDING
- [x] Build and refine T01 Task (A-share stock selection strategy) ✅ Config complete
- [x] Implement scheduled execution of stock selection ✅ Cron jobs active

### Long-term (6 Months)
- [ ] Build complete enterprise AI operating system
- [ ] Implement sales automation
- [ ] Implement marketing automation
- [ ] Implement investment decision automation

---

## 12. Multi-Agent Collaboration

| Agent | Role | Responsibilities |
|-------|------|-----------------|
| **Strategy Agent** | High-level planning | Strategic decisions, roadmap, priorities |
| **Research Agent** | Data gathering and analysis | Market research, data collection, analysis |
| **Execution Agent** | Task implementation | Coding, deployment, operations |
| **Monitoring Agent** | Track performance and alerts | KPIs, alerts, performance tracking |

---

## Current Focus

**T01 Task: A股龙头选股策略系统**

Configuration completed:
- [x] API data interface documentation - Tushare Pro access confirmed (limit_up/auction/top_list)
- [x] Stock selection logic details - Eleven-factor weights confirmed
- [x] Risk control recommendations - Default thresholds confirmed (MA3 bias 6%, auction gain <1% filter, etc.)
- [x] Backtest evolution suggestions - Evolution module ready (auto-execute every Sunday 20:00)

**Status**: ✅ T01 configuration complete, system is ready, waiting for trading day auto-execution

---

*The more you know, the better you can help. But remember — you're learning about a person, not building a dossier. Respect the difference.*

---

*Last Updated: 2026-03-07*
