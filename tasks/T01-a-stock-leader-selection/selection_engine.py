...
class T1AuctionEngine:
    ...
    def _load_auction_judgment_config(self) -> Dict:
        """加载竞价综合判断配置"""
        import json
        config_path = os.path.join(os.path.dirname(__file__), 'auction_selection_config.json')
        with open(config_path, 'r', encoding='utf-8') as f:
            return json.load(f)

    def run(self, date: str = None, t_day_stocks: List[Dict] = None, top_n: int = 2) -> List[Dict]:
        """
        执行T+1竞价选股流程
        ...
        # 7. 综合判断选择不超过2只股票
        config = self._load_auction_judgment_config()
        top_stocks = self._comprehensive_judgment(scored_stocks, market_risk, config)
    ...
    def _comprehensive_judgment(self, scored_stocks: list, market_risk: str, config: dict) -> list:
        """
        综合判断选择股票
        
        Args:
            scored_stocks: 已评分的股票列表
            market_risk: 市场风险等级 ('high', 'medium', 'low')
            config: 配置字典
        ...
        judgment_factors = config['judgment_factors']
        selected = []
        sector_count = {}
        
        # 1. 应用分数阈值过滤
        if judgment_factors['score_threshold']['enabled']:
            threshold = judgment_factors['score_threshold']['value']
            scored_stocks = [s for s in scored_stocks if s.get('final_score', 0) >= threshold]
            if not scored_stocks:
                return []
        
        # 2. 根据市场风险调整最大可选数量
        if judgment_factors['market_risk']['enabled']:
            if market_risk == 'high':
                max_count = judgment_factors['market_risk']['high_risk_max_selection']
            elif market_risk == 'medium':
                max_count = judgment_factors['market_risk']['medium_risk_max_selection']
            else:  # low
                max_count = judgment_factors['market_risk']['low_risk_max_selection']
        else:
            max_count = config['max_selections']
        
        # 3. 按分数排序
        sorted_stocks = sorted(scored_stocks, key=lambda x: x['final_score'], reverse=True)
        ...
        for i, stock in enumerate(sorted_stocks):
            ...
            stock_score = stock.get('final_score', 0)
            stock_sector = stock.get('sector', 'unknown')
            
            # 分数差距判断
            if judgment_factors['score_gap_judgment']['enabled']:
                if i == 0:  # 第一名
                    next_score = sorted_stocks[i+1].get('final_score', 0) if i+1 < len(sorted_stocks) else 0
                    if next_score > 0 and (stock_score - next_score) < judgment_factors['score_gap_judgment']['top_score_min_gap']:
                        print(f"   ⚠️ 第一名与第二名分数差距过小({stock_score - next_score:.1f})，增加选择数量")
                        max_count = min(max_count + 1, config['max_selections'])
                else:  # 后面的名次
                    if i == 1:  # 第二名
                        prev_score = sorted_stocks[i-1].get('final_score', 0)
                        if (prev_score - stock_score) > judgment_factors['score_gap_judgment']['secondary_score_min_gap']:
                            print(f"   ⚠️ 第二名与第一名分数差距过大({prev_score - stock_score:.1f})，停止选择")
                            break
            
            # 板块分散度判断
            if judgment_factors['sector_diversification']['enabled']:
                sector_count[stock_sector] = sector_count.get(stock_sector, 0) + 1
                if sector_count[stock_sector] > judgment_factors['sector_diversification']['max_from_same_sector']:
                    print(f"   ⚠️ 板块{stock_sector}选股数量已达上限，跳过{stock['ts_code']}")
                    continue
            
            # 技术形态偏好判断
            if judgment_factors['pattern_recognition']['enabled']:
                reason = stock.get('reason', '')
                avoid_pattern = any(pattern in reason for pattern in judgment_factors['pattern_recognition']['avoid_patterns'])
                if avoid_pattern:
                    print(f"   ⚠️ {stock['ts_code']} 包含需规避形态: {reason}")
                    continue
            
            selected.append(stock)
            print(f"   🎯 选中: {stock['ts_code']} {stock.get('stock_name', '')} - 分数: {stock.get('final_score', 0):.1f}")
        
        # 特殊情况处理：如果第一名分数极高，直接单独选择
        if len(selected) > 1 and judgment_factors['score_gap_judgment']['enabled']:
            top_score = selected[0].get('final_score', 0)
            second_score = selected[1].get('final_score', 0)
            if top_score - second_score > judgment_factors['score_gap_judgment']['secondary_score_min_gap'] * 2:
                print(f"   ⚠️ 第一名分数远超其他股票，仅选择第一名")
                return [selected[0]]
        
        print(f"✅ 综合判断完成，共选出{len(selected)}只股票")
        return selected
...
if __name__ == '__main__':
    ...
    # 测试T+1竞价选股
    print("\n测试 T+1竞价选股引擎...")
    auction_stocks = run_t1_auction_selection()
    
    if auction_stocks:
        print("\n竞价选股结果:")
        for s in auction_stocks[:3]:
            print(f"  {s['ts_code']} {s.get('stock_name', '')} - {s.get('final_score', 0):.1f}分")